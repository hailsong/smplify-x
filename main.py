...
# Usage : python main.py "filename" "gender"
# ex) python main.py ex_f_1 female

def main(filename_args : str, gender_args : str):
  # STEP 1 : Import image
  step_1_time = time.time()
  PROJECT_DIR = '/root/Dataset/smplify-x/'

  target_name = filename_args

  if not os.path.exists(os.path.join(PROJECT_DIR, 'input', target_name, 'images')):
    os.makedirs(os.path.join(PROJECT_DIR, 'input', target_name, 'images'))

  if not os.path.exists(os.path.join(PROJECT_DIR, 'input', target_name, 'keypoints')):
    os.makedirs(os.path.join(PROJECT_DIR, 'input', target_name, 'keypoints'))

  if not os.path.exists(os.path.join(PROJECT_DIR, 'input', target_name, 'openpose_images')):
    os.makedirs(os.path.join(PROJECT_DIR, 'input', target_name, 'openpose_images'))

  if not os.path.exists(os.path.join(PROJECT_DIR, 'input', target_name, 'smplifyx_results')):
    os.makedirs(os.path.join(PROJECT_DIR, 'input', target_name, 'smplifyx_results'))

  RES_DIR = os.path.join(PROJECT_DIR, 'input', target_name)
  ...
  test_img_path = os.path.join(PROJECT_DIR, 'inputImages')
  filename = target_name + '.jpg'

  test_img = load_img(os.path.join(test_img_path, filename))
  save_img(test_img, os.path.join(PROJECT_DIR, 'input', target_name, 'images', filename))
  ...
  KEYPOINTS_DIR = os.path.join(PROJECT_DIR, 'input', target_name , 'keypoints')
  OPENPOSE_IMAGES_DIR = os.path.join(PROJECT_DIR , 'input', target_name , 'openpose_images')
  ...
  gender = gender_args #@param ["neutral", "female", "male"]
  ...
  os.system('python smplifyx/main.py --config cfg_files/fit_smplx.yaml \
      --data_folder  /root/Dataset/smplify-x/input/{} \
      --output_folder /root/Dataset/smplify-x/input/{}/smplifyx_results \
      --visualize=False \
      --gender={} \
      --model_folder /root/Dataset/smplify-x/smplx/models \
      --vposer_ckpt /root/Dataset/smplify-x/vposer/vposer_v1_0 \
      --part_segm_fn smplx_parts_segm.pkl \
      --interpenetration=False'.format(target_name, target_name, gender))
  ...
  with open('/root/Dataset/smplify-x/input/{}/smplifyx_results/results/{}/000.pkl'.format(target_name, target_name), 'rb') as f:
      data = pickle.load(f)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'filename',
        type=str,
        help="Filename for smplify-x target"),
    parser.add_argument(
       'gender',
       type=str,
       help="Select gender, male of female"
    )

    args = parser.parse_args()

    main(args.filename, args.gender)
